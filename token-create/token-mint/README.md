...
こちらで作ったAssociated Token Accountに対してトークンを発行していきます。
トークンミントアカウント作成時にMint Authorityとして定義したアドレスのみがトークンを発行できます。
発行されたトークンはトークンアカウントで受け取ることができます。
では実際にトークンを発行するプログラムを書いていきましょう。
